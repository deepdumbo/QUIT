...
template <typename Model> struct SRCFit {
    ...
    QI::FitReturnType fit(const std::vector<Eigen::ArrayXd> &inputs, const Eigen::ArrayXd &fixed,
        ...
        QI::RegionContraction<Functor> rc(func, model.bounds_lo, model.bounds_hi, thresh,
                                          src_samples, src_retain, max_iterations, 0.02, src_gauss,
                                          false);
        if (!rc.optimise(v)) {
...
int main(int argc, char **argv) {
    ...
    args::ValueFlag<int> modelarg(parser, "MODEL", "Select model to fit - 2/3, default 3",
                                  {'M', "model"}, 3);
    args::Flag scale(parser, "SCALE", "Normalize signals to mean (a good idea)", {'S', "scale"});
    args::Flag use_src(parser, "SRC",
                       "Use flat prior (stochastic region contraction), not gaussian", {"SRC"});
    ...
    auto process = [&](auto model, const std::string &prefix) {
        if (simulate) {
            QI::SimulateModel<decltype(model), true>(input, model, {f0.Get(), B1.Get()},
                                                     input_paths.Get(), verbose, simulate.Get());
        } else {
