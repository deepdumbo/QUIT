...
template <typename T>
T Gaussian(const double &f0, const T T2b)
{
...
template <typename T>
QI_ARRAY(T)
Gaussian(const Eigen::ArrayXd &f0, const T T2b)
{
    return sqrt(1.0 / (2.0 * M_PI)) * T2b * exp(-pow(2.0 * M_PI * f0 * T2b, 2.0) / 2.0);
}
...
template <typename T>
QI_ARRAY(T)
Lorentzian(const Eigen::ArrayXd &f0, const T T2b)
{
...
template <typename T>
QI_ARRAY(T)
SuperLorentzian(const Eigen::ArrayXd &df0, const T T2b)
{
...
struct InterpLineshape
{
    ...
    template <typename T>
    QI_ARRAY(T)
    operator()(const Eigen::ArrayXd &f, const T T2) const
    {
